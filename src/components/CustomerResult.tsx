...
function Evidence({ ids, analysis }: { ids: string[]; analysis: SajuAnalysis }) {
  ...
  return <details className="evidence"><summary>왜 이렇게 나오나요?</summary><ul>{ids.map((id) => <li key={id}><code>{id}</code><span>{source.get(id) ?? "검증된 근거 식별자 · 상세 설명은 엔진 원본을 기준으로 합니다."}</span></li>)}</ul></details>;
}
...
function Periods({ analysis, current }: { analysis: SajuAnalysis; current: CurrentPeriodSelection }) {
  ...
  const initialYear = current.seunYear ?? seunAll[0]?.year ?? 0;
  ...
  return <><Section id="daeun" eyebrow="대운" title="10년 흐름"><div className="timeline" role="list" aria-label="대운 타임라인">{fortune.daeun.status === "implemented" && fortune.daeun.periods.map((period) => <button role="listitem" aria-pressed={period.index === daeunIndex} className={period.index === daeunIndex ? "active" : ""} key={period.index} onClick={() => setDaeunIndex(period.index)}><b>{period.pillar.stem}{period.pillar.branch}</b><span>{period.sourcePeriod.ageRange}</span><small>{FAVORABILITY[period.preference.role] ?? period.preference.role} · 변화 {ACTIVITY[period.activation.level]}</small></button>)}</div>{selectedDaeun && <div className="periodDetail"><h3>{selectedDaeun.pillar.stem}{selectedDaeun.pillar.branch} 대운</h3><div className="metricGrid"><Score label="선호도" score={selectedDaeun.preference.baseFavorabilityScore} level={selectedDaeun.preference.role} /><Score label="활성도" score={selectedDaeun.activation.score} level={selectedDaeun.activation.level} tone="activity" /></div><p>십성 {selectedDaeun.tenGodProfile.stemTenGod} · 주요 상호작용 {selectedDaeun.interactions.length}개</p></div>}</Section><Section id="seun" eyebrow="세운" title="해마다 달라지는 흐름"><div className="yearPicker" role="list" aria-label="세운 연도 선택">{seunAll.slice(0, 18).map((period) => <button role="listitem" aria-pressed={period.year === year} className={period.year === year ? "active" : ""} key={period.year} onClick={() => setYear(period.year)}><b>{period.year}</b><span>{period.pillar.stem}{period.pillar.branch}</span></button>)}</div>{seun && <SeunDetail period={seun} />}</Section><Wolun periods={wolun} /></>;
}
...
function Categories({ analysis, current }: { analysis: SajuAnalysis; current: CurrentPeriodSelection }) {
  const fortune = analysis.fortune as FortuneResult;
  if (fortune.categories.status !== "implemented" || fortune.synthesis.status !== "implemented") return null;
  const axes = fortune.synthesis.seun.find((item) => item.context.seunYear === current.seunYear) ?? fortune.synthesis.daeun[current.daeunIndex ?? 0] ?? fortune.synthesis.seun[0];
  ...
  return <><Section id="axes" eyebrow="종합 운 흐름" title="서로 다른 세 가지 축"><div className="metricGrid three"><Score label="지원 흐름" score={axes.favorability.score} level={axes.favorability.level} /><Score label="변화 활성도" score={axes.activation.score} level={axes.activation.level} tone="activity" /><Score label="오행 정렬도" score={axes.transformationAlignment.adjustedScore} level={axes.transformationAlignment.level} /></div></Section><Section id="categories" eyebrow="분야별 흐름" title="생활 영역별로 살펴보기"><CategoryGrid category={category} /><CategoryDetails category={category} /><Evidence ids={category.evidence.slice(0, 4)} analysis={analysis} /></Section></>;
}
...
function Interpretation({ state, onRequest, onRetry, analysis }: { state: InterpretationUiState; onRequest: () => void; onRetry: () => void; analysis: SajuAnalysis }) {
  return <Section id="interpretation" eyebrow="AI 종합 해석" title="계산 근거를 문장으로 읽기">{state.status === "not_requested" && <div className="emptyState"><p>검증된 엔진 결과를 바탕으로 해석을 생성할 수 있어요.</p><button onClick={onRequest}>AI 해석 생성하기</button></div>}{state.status === "pending" && <div className="skeleton" role="status" aria-label="AI 해석 생성 중"><i /><i /><i /></div>}{state.status === "failed" && <div className="errorPanel" role="alert"><strong>AI 해석만 불러오지 못했어요</strong><p>위의 사주 계산 결과는 정상이며 그대로 확인할 수 있습니다.</p><button onClick={onRetry}>해석 다시 시도</button></div>}{state.status === "completed" && <Report report={state.report} analysis={analysis} />}</Section>;
}
