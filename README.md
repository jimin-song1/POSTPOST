...
음력 변환, 출생시간 미상 원국, 해외 시간대·경도 보정과 격국 성패·용신·신살·운 계산은 아직 구현하지 않았으며 응답에 `not_implemented` 또는 `unsupported_input`으로 표시합니다.
