...
describe("CUSTOMER_LIFETIME_UI_V2 report",()=>{
  ...
  it("ships long Korean synthetic copy for browser layout QA",async()=>{const preview=await readFile("src/app/dev/lifetime-report/page.tsx","utf8");expect(preview).toContain("공개 합성 입력");expect(preview.length).toBeGreaterThan(1800);expect(preview).not.toContain("OPENAI_API_KEY");});
