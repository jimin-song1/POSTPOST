...
describe("CUSTOMER_LIFETIME_UI_V2 input", () => {
  ...
  it("names each picker in customer language when it flips above",async()=>{const date=await dateSource(),time=await timeSource(),css=await readFile("src/app/globals.css","utf8");expect(date).toContain("생년월일 선택</p>");expect(time).toContain("태어난 시각 선택</p>");expect(date).toContain("picker-title");expect(time).toContain("picker-title");expect(css).toContain(".pickerMenu::before");expect(css).toContain(".hybridPicker.opensAbove .pickerMenu::before");});
