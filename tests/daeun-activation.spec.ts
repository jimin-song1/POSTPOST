...
describe("SYNTHETIC_DAEUN_GENERATION_AND_ACTIVATION_V1",()=>{
  ...
  it("AD-AK: preserves all upstream axes, is deterministic, and integrates only daeun",()=>{
    ...
    expect(first.fortune).toMatchObject({status:"partial",daeun:{status:"implemented"},seun:{status:"not_implemented"},
      wolun:{status:"not_implemented"},synthesis:{status:"not_implemented"}});
